...
--
-- Run this AFTER `001_initial.sql`. Best executed in the Supabase SQL editor
-- (which runs as service_role and bypasses RLS).
--
-- The seed inserts 20 plausible submissions across teams and components, with
-- intentional duplicates and cross-team patterns so the AI dashboard's
-- groupings and recommendations are visibly meaningful even in demo mode.
--
-- It is safe to re-run: it only inserts if there are fewer than 5
-- submissions already in the table.
